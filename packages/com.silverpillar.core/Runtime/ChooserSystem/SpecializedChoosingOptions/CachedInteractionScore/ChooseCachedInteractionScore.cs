...
{
    [Serializable]
    public class ChooseCachedInteractionScore : ICachedInteractionScore, IChoose
    {
        [SerializeField]
        private HowToCalculateScore m_HowToCalculateScore = HowToCalculateScore.ChooseFirst;
        [OdinSerialize, ShowInInspector]
        private IChooseData<ICachedInteractionScore> m_Chooser;
        public IChooseData<ICachedInteractionScore> Chooser => m_Chooser;
        private List<ICachedInteractionScore> m_ChosenScores;
        private List<float> m_Scores;
        private GameObject m_Self;
        private bool m_InitializedCorrectly = false;

        public ChooseCachedInteractionScore() { }
        public ChooseCachedInteractionScore(ChooseCachedInteractionScore other)
        {
        ...
        {
            return new ChooseCachedInteractionScore(this);
        }
        ...
        public float CalculateScore(GameObject target)
        {
            ...
            for (int i = 0; i < m_ChosenScores.Count; i++)
            {
                ...
                {
                    m_Scores.Add(m_ChosenScores[i].CalculateScore(target));
                }
            }
