...
{	
	...
	/// </summary>
	[AddComponentMenu("Corgi Engine/Spawn/Enhanced Finish Level")]
	public class EnhancedFinishLevel : ButtonActivated 
	{

		[MMInspectorGroup("Finish Level", true, 22)]

		[SerializeField]
		private RandomChoice m_RandomChoice;
        /// the (exact) name of the level to go to 
        [Tooltip("the (exact) name of the level to go to ")]
		public List<string> PossibleLevelNames = new();
		...
		public virtual void GoToNextLevel()
		{
			...
			switch (m_RandomChoice)
			{
				case RandomChoice.SeededRandom:
                    currentLevel = PossibleLevelNames[RandomManager.Instance.GetRandomInRange(0, PossibleLevelNames.Count)];

                    break;
				case RandomChoice.Random:
                    currentLevel = PossibleLevelNames[UnityEngine.Random.Range(0, PossibleLevelNames.Count)];
			...
			{
				LevelManager.Instance.GotoLevel(currentLevel, (DelayBeforeTransition == 0f));
			}
			else
			{
